...
public class Ding2{
	...
	public Ding2(HttpServletResponse response, String chartTitle,// 图片标题
	...
	public void drawChart(int hangLeng,int zongLength) {
		...
		for(int k = 0;k < dingValue1.length;k++){
				...
				max = Math.max(max, dingValue1[k][second]);
				min = Math.min(min, dingValue1[k][second]);
		...
		for(int k = 0;k < dingValue1.length;k++){
			...
			if (dingValue1[k][first1] == min) {
				...
				double tem = (double) ((dingValue1[k][first1] - min) / (max - min));
		...
		// 转换数值把数值变成椭圆的宽value[][]
		double[][] value2 = new double[dingValue1.length][dingValue1[0].length];
		for (int k = 0; k < dingValue1.length; k++) {
			for (int kk = 0; kk < dingValue1[k].length; kk++) {
				if (dingValue1[k][kk] == min) {
					if (allMin == 0) {
						value2[k][kk] = 0.01 * hSpace;// 最小的值
					} else {
						value2[k][kk] = Math.sqrt(allMin) * vSpace;
					}
				} else {
					double tem = (double) ((dingValue1[k][kk] - min) / (max - min));
					if (allMax != 0) {
						// 如果数字大于tempMax
						if (tem > allMax) {
							tem = allMax;
					...
					value2[k][kk] = Math.sqrt(tem) * hSpace;
				}
			}
		...
		for (int a = 0; a < value1.length; a++) {
			for (int aa = 0; aa < value1[a].length; aa++) {
				ellipse.setFrame((hSpace - value2[a][aa]) / 2 + a * hSpace+strLength, (vSpace-value1[a][aa])/2+aa
						* vSpace+hightLength, value2[a][aa], value1[a][aa]);
				g2d.fill(ellipse);
