...
import org.plot.chart.impl.XYDisperse;
...
public class XYDisperseChart extends HttpServlet {

	public XYDisperseChart() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String[] xScaleB = trans.getXscale();
		int[] xScaleA = new int[xScaleB.length] ;
		for(int i =0;i<xScaleB.length ;i++){
			xScaleA[i] = pi.parseInt(xScaleB[i]) ;
		}
		
//		int[][] xScale = trans.getYea();
		...
		try {
			...
			// 调用线段绘制的类
			Chart chart = new XYDisperse(session,// 固定格式传入session
					// ******必须传入******
					chartTitle,// 图片标题
					LineTitle,// 各个线段的名称
					xTitle, // X轴的名称
